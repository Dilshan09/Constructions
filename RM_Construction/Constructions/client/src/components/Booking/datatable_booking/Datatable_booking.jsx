...
const Datatable_booking = () => {
  ...
  const { data, loading, error } = useFetch("http://localhost:8070/rm/");
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete(`http://localhost:8070/rm/delete/${id}`);
        ...
        return (
          <div className="cellAction">
            <Link
              ...
              type="button"
              to={`/booking/updateBookingga/${params.row._id}`}
            >
              Update
            </Link>
            <div
  ...
  const downloadPdf = () => {
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text("RM Constructions", 14, 22);
    doc.setFontSize(12);
    doc.text(
    ...
    doc.autoTable({
      ...
      body: list.map((item) => [
        // item.bookingid,
        /// real item variable (in form useState)
        item.name,
        item.phone,
    ...
    doc.save("Delivery Informations.pdf"); /// download pdf name
  };
